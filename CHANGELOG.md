...
- デモ・README が参照するコア Haori.js を `0.52.0` から `0.53.0` に更新しました（`demo/cdn.html`、`demo/admin-table.html`、`demo/modal-copy.html`、`demo/dialog-label.html`、README / README.ja の CDN 利用例）。
- **コア 0.53.0 に、このパッケージへの破壊的変更はありません。** 内容は、交差監視トリガー（`data-intersect-*`）が抑止の間に来た交差を捨てたまま拾い直さない問題と、再同期のたびに監視を作り直してしまう問題の修正です。`src/` には `data-intersect-*` が 1 件もありません。
- **デモの `demo/admin-table.html`（無限スクロールの一覧）だけが `data-intersect-*` を使います。** `data-intersect-disabled` で読み込み中と終端を抑止しているため、抑止が解けた時点で取りこぼした交差を拾い直すようになります。追い読みが途中で止まらなくなる方向の変化で、表示や操作の手順は変わりません。
...
- デモ・README が参照するコア Haori.js を `0.49.0` から `0.50.0` に更新しました（`demo/cdn.html`、`demo/admin-table.html`、`demo/modal-copy.html`、`demo/dialog-label.html`、README / README.ja の CDN 利用例）。
- **コア 0.50.0 に、このパッケージへの破壊的変更はありません。** 内容は、まとめて追加したノードの 2 つめ以降が取り込まれない問題、`data-external` なしで入力欄を包む連携で値が保存されない問題、`data-each` で選択肢を描く `<select>` の初期値が外部ウィジェットへ伝わらない問題、初期表示の取得がフォームの初期値を待たない問題、取り外しで子の断片が半分残る問題の修正です。**このパッケージは `data-enhance`・`data-external`・`data-fetch-form`・`data-url-param` のいずれも使っていません**（`src/`、`demo/`、`playwright/`、`tests/`、README を確認済み）。
- ダイアログの組み立てで複数のノードをまとめて追加している箇所（`src/dialog.ts`）は、コアの修正で**取り込みがより確実になる側**の変更です。単体 83 件と型検査は通っています。
...
- デモ・README が参照するコア Haori.js を `0.47.3` から `0.47.4` に更新しました（`demo/cdn.html`、`demo/admin-table.html`、`demo/modal-copy.html`、`demo/dialog-label.html`、README / README.ja の CDN 利用例）。
- **コア 0.47.4 に、このパッケージへの破壊的変更はありません。** 内容は、祖先が一覧やレコードを持つ編集フォーム（`data-form-arg` を宣言しない構成）で、入力すると `id` などのフィールドが落ち、以降の再取得が届かなくなる問題の修正です。**このパッケージには `<form>` も `data-form` コンテナも 1 件もなく**（`demo/`、`src/` を確認済み）、フォームの双方向コミットを経由する箇所がないため影響を受けません。
