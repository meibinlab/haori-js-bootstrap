...
- Haori 本体を改変せず、グローバル置換または明示的初期化によって差し替える構成を優先する。
...
- ESM と IIFE を正式配布形式とし、IIFE は自動インストール、ESM は install の明示呼び出しを正式導線とする。
...
| API | 目的 | 必須度 |
| ---- | ---- | ---- |
| install(options) | window.Haori の差し替えと初期設定適用 | 高 |
...
| installBootstrapHaori | グローバル差し替えと初期化を担当 |
...
### 9.3 自動インストールと明示 install の扱い

- IIFE 版は、利用者の期待どおり「Haori と一緒に読み込むと差し替わる」体験を実現するため、自動インストールを正式採用する。
- ただし prerequisites が不足している場合はページ全体を停止させず、console.warn を出して自動インストールを見送る。
- ESM 版は import 時の副作用を避けるため、自動インストールを行わず install の明示呼び出しのみを正式導線とする。
- IIFE 版でも window.HaoriBootstrap.install と window.HaoriBootstrap.uninstall を公開し、再インストールやテスト制御を可能にする。
...
- IIFE 自動インストール時はページ停止を避けるため、prerequisites 不足を console.warn で通知し、自動インストールを見送る。
...
- target が input、select、textarea の場合は target 直後に invalid-feedback 互換の所有コンテナを生成し、対象 control へ所有印付きの is-invalid 状態を付与する。
...
- IIFE 版が prerequisites 充足時のみ自動インストールし、不足時は警告のみで停止しないこと
- ESM 版が import 時に副作用を持たず、install 呼び出し時のみ差し替えること
...
| Toast のキュー制御 | 同時表示数、積み上げ方向、自動消去秒数の細かな既定値 | UX と examples に影響 |
| TypeScript 対応時期 | 初期版を JavaScript で始めた後、型定義をいつ導入するか | 開発体験と公開 API ドキュメントに影響 |
| サポートブラウザ | Bootstrap 5.3 系の前提をどこまで README で明示するか | テスト範囲と公開条件に影響 |
