...
本書は、Haori.js の Bootstrap 向け拡張ライブラリである Haori.js Bootstrap の初期設計案を整理することを目的とする。現時点では既存実装が存在しないため、README と利用者から与えられた前提、ならびに Haori.js の公開仕様として確認できている範囲のみを根拠に、実装前の判断材料を残す。
