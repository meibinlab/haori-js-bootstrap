...
本書は、Haori.js の Bootstrap 向け拡張ライブラリである Haori.js Bootstrap の初期設計案を整理することを目的とする。実装は src と tests に反映済みだが、本書では README と利用者から与えられた前提、ならびに Haori.js の公開仕様として確認できている範囲を根拠に、初期設計判断の記録を残す。
...
既存画面内の `.modal` 要素に対して Haori.openDialog と Haori.closeDialog を呼び出し、Bootstrap Modal として開閉する。引数には `.modal` 自身またはその子孫（例: モーダル内の閉じるボタン）を渡すことができ、`.modal` でない要素が渡された場合は祖先方向で最も近い `.modal` を対象に解決する。任意要素を破壊的に `.modal` 化することはせず、解決できない場合は元の Haori 実装へフォールバックする。
...
| confirm(message) | 確認ダイアログ | Modal 上の OK / Cancel | Promise<boolean>。OK のみ true、Cancel / Esc / 障害時は false。backdrop click では閉じない。message 中の `\n` は改行として表示 |
...
| openDialog(element) | 対象 Modal の表示 | 対象を Modal として開く | Promise<void>。element は `.modal` 自身またはその子孫の HTMLElement 前提。非 `.modal` の場合は祖先方向で最も近い `.modal` に解決し、解決できなければ要素を modal 化せず元実装へフォールバックする |
| closeDialog(element) | 対象 Modal の非表示 | Modal を閉じる | Promise<void>。Bootstrap instance を再利用する。element の解決規則は openDialog と同じ |
...
| toastContainerSelector | Toast 配置先。省略時は body 配下の右下固定コンテナを使用 | 確定 |
...
- 現在の doc/ 配下の初期設計書は初期設計の記録として維持し、必要があれば docs/ja への再配置を別途検討する。
...
- confirm は OK 操作のみ true を返し、Cancel、Esc、閉じるボタン、Bootstrap 障害時はすべて false に正規化する。backdrop click では閉じない。
- dialog、confirm、toast の message は常に textContent 相当で描画し、innerHTML は使用しない。改行表示は `white-space: pre-line` で扱う。
...
- confirm が Promise<boolean> を返し、OK のみ true、Cancel / Esc / 障害時が false であること
- confirm と dialog が backdrop click では閉じないこと
